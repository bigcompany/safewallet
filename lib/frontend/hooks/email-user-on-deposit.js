var debug = require('debug')('alert'),
    ...
    template = require('fs').readFileSync(__dirname + '/../email/deposit-confirmation.txt').toString();
...
wallet.on('deposit', function (data){
  user.find({ name: data.owner }, function (err, users){
    ...
    template = template.replace('{{amount}}', data.deposit.amount);
    template = template.replace('{{currency}}', data.deposit.currency);
    ...
    if (typeof _user.email !== 'undefined' && _user.email.length > 0) {
       ...
       email.send(message, function(err, result){
         ...
         debug('deposit confirmation email sent to ' + data.email);
       });
    }
  });
});
