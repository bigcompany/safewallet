...
module['exports'] = function (app) {
  ...
  app.post('/generateAddress', function (req, res, next) {
         ...
         clients[currency].cmd('getnewaddress', record.owner, function (err, result) {
           if (err) {
             if (err.code === "ECONNREFUSED") {
               console.log('Unable to connect to ' + currency + ' client daemon @ ' + JSON.stringify(clients[currency].rpc.opts, true, 2), err)
