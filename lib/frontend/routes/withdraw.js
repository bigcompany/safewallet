...
module['exports'] = function (app) {
  ...
  app.post('/withdraw', function(req, res, next) {
    ...
    wallet.find({ owner: req.session.name }, function (err, result) {
      ...
      wallet.withdraw({ id: result[0].id, currency: req.resource.params.currency, amount: req.resource.params.amount }, function (err, result) {
        if (err) {
        ...
        return res.end('withdrawal successful');
      });
    });
  });
