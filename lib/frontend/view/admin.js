...
module['exports'] = function (options, callback) {
  ...
  if (options.transactionID) {
    // get transaction
    var _status = ["pending", "blocked", "ready", "processed"];
    ...
    arr.push({ type: entry.type, owner: entry.owner, date: dt, status: entry.status, address: '<a target="_blank" href="' + link + '">' + entry.address + '</a>', currency: entry.currency, amount: entry.amount });
    arr.unshift({ date: 'date', owner: 'owner', type: 'type', status: 'status', address: 'address', currency: 'currency', amount: 'amount' });
  ...
  if (options.ledger.length === 0) {
    ...
    ledger.forEach(function(item){
      ...
      arr.push({ type: item.type, owner: item.owner, date: dt, status: item.status, address: '<a target="_blank" href="' + link + '">' + item.address + '</a>', currency: item.currency, amount: item.amount });
    });
    arr.unshift({ date: 'date', owner: 'owner', type: 'type', status: 'status', address: 'address', currency: 'currency', amount: 'amount' });
    var str = html.render({ transactions: arr }, tmpl);
    $('.ledger').html(str);
